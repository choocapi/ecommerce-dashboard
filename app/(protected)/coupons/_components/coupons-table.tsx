...
export function CouponsTable({ data }: DataTableProps) {
  ...
  // Column filters state (coupons have type and isActive filters)
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([
    ...(typeFilter !== null ? [{ id: "type", value: [typeFilter] }] : []),
  ...
  const {
    ...
    queryKey: ["coupons", "list", { page: pageIndex, size: pageSize, filters }],
    queryFn: () => couponService.list(pageIndex, pageSize, filters),
    placeholderData: (previousData) => previousData,
  });

  const coupons = couponsData?.content || [];
  const pageCount = couponsData?.totalPages || 0;
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={couponsColumns.length} className="h-24 text-center">
                  {isLoading
