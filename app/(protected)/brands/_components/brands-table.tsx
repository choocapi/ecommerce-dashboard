...
export function BrandsTable({ data }: DataTableProps) {
  ...
  // Column filters state (brands don't have additional filters beyond search)
  ...
  const {
    ...
    queryKey: ["brands", "list", { page: pageIndex, size: pageSize, filters }],
    queryFn: () => brandService.list(pageIndex, pageSize, filters),
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={brandsColumns.length} className="h-24 text-center">
                  {isLoading
