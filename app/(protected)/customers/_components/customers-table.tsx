...
export function CustomersTable({ data }: DataTableProps) {
  ...
  // Column filters state (customers have status and verified filters)
  ...
  const filters = useMemo(() => {
    const result: any = {
      role: RolesEnum.CUSTOMER, // Always filter for customer role
    };
  ...
  const {
    ...
    queryKey: ["customers", "list", { page: pageIndex, size: pageSize, filters }],
    queryFn: () => userService.list(pageIndex, pageSize, filters),
    placeholderData: (previousData) => previousData,
  });

  const customers = customersData?.content || [];
  const pageCount = customersData?.totalPages || 0;
