...
export function SuppliersTable({ data }: DataTableProps) {
  ...
  // Column filters state (suppliers don't have additional filters beyond search)
  ...
  const {
    ...
    queryKey: ["suppliers", "list", { page: pageIndex, size: pageSize, filters }],
    queryFn: () => supplierService.list(pageIndex, pageSize, filters),
    placeholderData: (previousData) => previousData,
  });

  const suppliers = suppliersData?.content || [];
  const pageCount = suppliersData?.totalPages || 0;
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows?.length ? (
              ...
              <TableRow>
                <TableCell colSpan={suppliersColumns.length} className="h-24 text-center">
                  {isLoading
